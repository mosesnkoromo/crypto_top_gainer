...
Whapi returns HTTP 200 with {"sent":true} on success.
"""
...
class WhatsAppSender:
    ...
    def __init__(self, cfg: WhatsAppConfig):
        ...
        log.info("WhatsApp sender ready (Whapi) — number: %s", self._number)

    def send(self, message: str) -> bool:
        for attempt in range(1, self._cfg.retry_attempts + 1):
            success, should_retry = self._try_send(message, attempt)
            if success:
                return True
            if not should_retry or attempt == self._cfg.retry_attempts:
                break
    ...
    def _try_send(self, message: str, attempt: int) -> tuple[bool, bool]:
        try:
            ...
            # Whapi success = HTTP 200 or 201 with "sent": true
            ...
            if resp.status_code in (401, 403):
                log.error("Auth error %d — check WHAPI_TOKEN in .env", resp.status_code)
                return False, False
