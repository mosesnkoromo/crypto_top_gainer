"""
src/trading/binance_trader.py — Institutional Scalp v2.2
- Added rate limiter (20 calls/min) to prevent 418 bans
- Cached positions/orders with TTL to reduce polling
- Fixed duplicate TP2 placement in futures execution
- Added exponential backoff on 418/429 errors
...
# Fallbacks (simplified)
_SPOT_FALLBACKS = ["https://api1.binance.com", "https://api2.binance.com", "https://api3.binance.com"]
_FUT_FALLBACKS  = ["https://fapi1.binance.com", "https://fapi2.binance.com", "https://fapi3.binance.com"]


# ------------------------------------------------------------------
# Rate Limiter (prevents 418 bans)
# ------------------------------------------------------------------
class RateLimiter:
...
class BinanceTrader:
    ...
    def __init__(
        ...
        # --- Rate limiter (critical for ban prevention) ---
        self._rate_limiter = RateLimiter(max_calls_per_minute=20)

        # --- Caches to reduce polling ---
        self._position_cache = {"data": [], "timestamp": 0}
        self._order_cache = {"data": [], "timestamp": 0}
        self._balance_cache = {"data": {}, "timestamp": 0}
        self._cache_ttl = 3  # seconds

        # Determine base URL with fallback
        if mode == "futures":
            primary = _FUT_BASE if live else _FUT_TEST
            fallbacks = _FUT_FALLBACKS if live else []
        else:
            primary = _SPOT_BASE if live else _SPOT_TEST
            fallbacks = _SPOT_FALLBACKS if live else []
        ...
        if live:
            if self._test_base_url(self._base):
                ...
                log.warning("Primary endpoint unreachable, trying fallbacks...")
                for fb in fallbacks:
                    self._base = fb
                    if self._test_base_url(self._base):
    ...
    def _test_base_url(self, base_url: str) -> bool:
        try:
            ...
            resp = _requests.get(f"{base_url}{path}", timeout=5)
            return resp.status_code == 200
        except Exception:
            return False

    # ------------------------------------------------------------------
    # Futures symbol normalization (unchanged)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Cached getters (reduce polling)
    # ------------------------------------------------------------------
    def get_positions(self, force_refresh: bool = False) -> list:
        if self._mode != "futures":
            return []
        now = time.time()
        if not force_refresh and (now - self._position_cache["timestamp"]) < self._cache_ttl:
            return self._position_cache["data"]
        try:
            resp = self._req("GET", "/fapi/v2/positionRisk", {})
            if not isinstance(resp, list):
                return []
            positions = [p for p in resp if isinstance(p, dict) and float(p.get("positionAmt", 0)) != 0]
            self._position_cache = {"data": positions, "timestamp": now}
            return positions
        except Exception as e:
            log.error("Positions error: %s", e)
            return self._position_cache["data"]  # return stale

    def get_open_orders(self, symbol: str = None, force_refresh: bool = False) -> list:
        now = time.time()
        if not force_refresh and (now - self._order_cache["timestamp"]) < self._cache_ttl:
            orders = self._order_cache["data"]
            if symbol:
        ...
        try:
            ...
            if self._mode == "futures":
                ...
                orders += algo_list
            self._order_cache = {"data": orders, "timestamp": now}
            if symbol:
                return [o for o in orders if o.get("symbol") == symbol]
            return orders
        except Exception as e:
            log.error("Open orders error: %s", e)
            return self._order_cache["data"]

    def get_available_balance(self, force_refresh: bool = False) -> float:
        now = time.time()
        if not force_refresh and (now - self._balance_cache["timestamp"]) < self._cache_ttl:
            return self._balance_cache["data"].get("available_balance", 0.0)
        try:
            if self._mode == "futures":
                resp = self._req("GET", "/fapi/v2/balance", {}) or []
                for a in resp:
                    if a.get("asset") == "USDT":
                        bal = float(a.get("availableBalance", 0))
                        self._balance_cache = {"data": {"available_balance": bal}, "timestamp": now}
                        return bal
            else:
                resp = self._req("GET", "/api/v3/account", {}) or {}
                for a in resp.get("balances", []):
                    if a["asset"] == "USDT":
                        bal = float(a["free"])
                        self._balance_cache = {"data": {"available_balance": bal}, "timestamp": now}
                        return bal
    ...
    def execute_signal(self, signal, balance_usdt: float) -> TradeResult:
        ...
        # Dynamic risk sizing
        _open_count = len(self.get_positions(force_refresh=True))
        cap = [1.0, 0.85, 0.70, 0.60, 0.50, 0.45, 0.40][min(_open_count, 6)]
        _ml_p = float(getattr(signal, "ml_prob", 0) or 0)
        _sn_p = float(getattr(signal, "sniper_conf", 1) or 1)
        conf = 1.2 if (_ml_p >= 0.75 or _sn_p >= 0.90) else (1.0 if (_ml_p >= 0.55 or _sn_p >= 0.65) else 0.7)
        dyn_risk = max(min(self._risk * cap * conf, 2.5), 0.5) * risk_mult

        if getattr(signal, "tag", "") == "REDUCED_RISK" or "REDUCED_RISK" in getattr(signal, "action", ""):
            dyn_risk *= 0.5
        ...
        try:
            if self._mode == "spot":
                ...
                # Pass tp2/tp3 explicitly to avoid duplicate placement
                r = self._futures(futures_sym, signal, qty, info, risk_usdt, pos_usdt,
                                  tp2=signal.tp2, tp3=signal.tp3)
            if r.success:
                self._daily_trades += 1
                # Invalidate caches after trade
                self._position_cache["timestamp"] = 0
                self._order_cache["timestamp"] = 0
            return r
        except Exception as e:
    ...
    # ------------------------------------------------------------------
    # Futures execution (fixed duplicate TP2)
    # ------------------------------------------------------------------
    def _futures(self, sym, signal, qty, info, risk_usdt, pos_usdt, tp2=None, tp3=None) -> TradeResult:
        ...
        # 70% market + 30% limit at slight pullback
        ...
        # TP orders (using passed tp2/tp3)
        ...
        # Use passed tp2/tp3 (avoid duplicate)
        if tp2 is not None and tp2 > 0:
            tp2_id = _place_tp("TP2", self._fp(tp2, info), q_tp2)
        if tp3 is not None and tp3 > 0:
            tp3_id = _place_tp("TP3", self._fp(tp3, info), q_tp3)

        # SL placement with immediate trigger check
        ...
        if raw_sl_p > 0:
            ...
            if sl_qty >= info.min_qty:
                ...
                if isinstance(sl_r, dict) and (sl_r.get("algoId") or sl_r.get("orderId")):
                    ...
                    log.info("Futures SL (algoOrder) @ %.6g id=%s ✅", raw_sl_p, sl_id)
                else:
                    ...
                    if isinstance(sl_r2, dict) and "orderId" in sl_r2:
                        ...
                        log.info("Futures SL (STOP_MARKET) @ %.6g id=%s ✅", raw_sl_p, sl_id)
                    else:
                        log.warning("Futures SL failed — position runs without SL ⚠️")
        ...
        self._recently_placed[sym] = time.time()
    ...
    # ------------------------------------------------------------------
    # Spot execution (unchanged, but uses rate limiter)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Price/quantity helpers (unchanged)
    # ------------------------------------------------------------------
    ...
    def _pre_flight(self, balance: float) -> tuple[bool, str]:
        ...
        if balance < 5:
            return False, f"Balance ${balance:.2f} too low"
    ...
    # ------------------------------------------------------------------
    # API request with rate limiting and 418/429 backoff
    # ------------------------------------------------------------------
    def _req(self, method: str, path: str, params: dict) -> dict | None:
        ...
        p["recvWindow"] = 20000
        backoff = 1
        for attempt in range(3):  # max 3 retries on 418/429
            ...
            try:
                ...
                log.warning("Binance %s %s timeout (attempt %d/3)", method, path, attempt + 1)
                time.sleep(backoff)
                backoff *= 2
                continue
            ...
            if resp.status_code in (418, 429):
                ...
                log.error("Binance %s %s → %d: %s", method, path, resp.status_code, resp.text[:300])
                wait = 60 * (attempt + 1)  # 60, 120, 180 seconds
                log.warning("Rate limit / ban detected. Sleeping %ds before retry...", wait)
                time.sleep(wait)
                continue

            if resp.status_code not in (200, 201):
                log.error("Binance %s %s → %d: %s", method, path, resp.status_code, resp.text[:400])
                try:
                    body = resp.json()
                except Exception:
                    return {"code": resp.status_code, "msg": resp.text[:200]}
                if body.get("code") == -1021 and attempt < 2:
    ...
    # ------------------------------------------------------------------
    # Other helpers (unchanged but using cache)
    # ------------------------------------------------------------------
    ...
    def _order(self, params: dict) -> dict:
        # Simplified – kept for backward compatibility, but _futures uses direct _req
        return self._req("POST", "/fapi/v1/order", params) or {}

    def get_balance(self) -> dict:
        # Use cached version
        avail = self.get_available_balance()
        return {"available_balance": avail, "wallet_balance": avail, "unrealised_pnl": 0.0, "error": ""}

    def cancel_all_orders(self, symbol: str = None) -> dict:
        cancelled = []
        try:
            syms = [symbol] if symbol else list({o["symbol"] for o in self.get_open_orders(force_refresh=True) if "symbol" in o})
            for s in syms:
            ...
            log.error("Cancel orders error: %s", e)
    ...
    def protect_open_positions(self, signal_lookup: dict) -> list:
        ...
        try:
            orphans = self.cancel_all_orders()
            if orphans:
